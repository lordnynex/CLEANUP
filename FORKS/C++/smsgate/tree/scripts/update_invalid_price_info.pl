...
my $query = 	'SELECT message_status."REQUESTID", "MESSAGEID", tariff, message_status."TO", "STATUS" FROM '.
	 	...
	 	'and "PARTNERPRICE"<0 and smsrequest."WHEN">ts2int(TIMESTAMP \'2011-03-01\');';
...
while ( <TARIFF_OUT> ) {
  ...
  my $update_query = "update message_status set \"PARTNERPRICE\"='".$price."' where \"REQUESTID\"=".$reqid." and \"MESSAGEID\"=".$msgid.";";
